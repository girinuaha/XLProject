...
$(document).ready(function(){
	...
	$('#check').on('click', function() {
		...
		} else if ($("#touchpoint").val() == "") {
			document.getElementById('touchpoint').focus();
			$('#invalidTouchpoint').show();
			
			$('#touchpoint').keyup(function() {
				if($(this).val() == ''){
					$('#invalidTouchpoint').show();
				}else{
					$('#invalidTouchpoint').hide();
				}
			});
		} else if ($("#homepoc").val() == "") {
			document.getElementById('homepoc').focus();
			$('#invalidLocation').show();
			
			$('#homepoc').keyup(function() {
				if($(this).val() == ''){
					$('#invalidLocation').show();
				}else{
		...
		function setFieldDetail(data){
			...
			createHTML += '</div>';
			createHTML += '<div class="form-group col-sm-6">';
			createHTML += '<label>Rule ID</label>';
			createHTML += '<input class="form-control" value="'+data.Response.RuleID+'" disabled>';
			createHTML += '</div>';
			createHTML += '</div>';
			
			createHTML += '<div class="row">';
			createHTML += '<div class="form-group col-sm-6">';
			createHTML += '<label>Priority</label>';
			createHTML += '<input class="form-control" value="'+data.Response.Priority+'" disabled>';
			createHTML += '</div>';
			createHTML += '<div class="form-group col-sm-6">';
			createHTML += '<label>Action Code</label>';
			createHTML += '<input class="form-control" value="'+data.Response.ActionCode+'" disabled>';
			createHTML += '</div>';
			createHTML += '</div>';
